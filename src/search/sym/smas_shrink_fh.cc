...
SMASShrinkFH::SMASShrinkFH(const Options &opts) :
		SMASShrinkBucketBased(opts), f_start(
				HighLow(opts.get_enum("shrink_f"))), h_start(
...
void SMASShrinkFH::partition_into_buckets(const SymSMAS &abs,
...
static void collect_f_h_buckets(FHIterator begin, FHIterator end,
		SMASShrinkFH::HighLow h_start, vector<Bucket> &buckets) {
...
void SMASShrinkFH::ordered_buckets_use_map(const SymSMAS &abs,
		vector<Bucket> &buckets) const {
...
void SMASShrinkFH::ordered_buckets_use_vector(const SymSMAS &abs,
		vector<Bucket> &buckets) const {
