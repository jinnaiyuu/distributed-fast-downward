...
class LabelReducer {
	...
	OperatorSignature build_operator_signature(const Operator &op,
			OperatorCost cost_type, const std::vector<bool> &var_is_used) const;
public:
	LabelReducer(const std::vector<const Operator *> &relevant_operators,
			const std::vector<int> &pruned_vars, OperatorCost cost_type);
	LabelReducer(const std::vector<const Operator *> &relevant_operators,
			const std::set<int> &pruned_vars, OperatorCost cost_type);
	~LabelReducer();
