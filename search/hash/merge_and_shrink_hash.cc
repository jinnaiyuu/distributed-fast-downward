#include "merge_and_shrink_hash.h"
...
MergeAndShrinkHash::MergeAndShrinkHash(const Options &opts)
    : Heuristic(opts), DistributionHash(opts),
...
MergeAndShrinkHash::~MergeAndShrinkHash() {
}

void MergeAndShrinkHash::dump_options() const {
    cout << "Merge strategy: ";
...
void MergeAndShrinkHash::warn_on_unusual_options() const {
...
Abstraction *MergeAndShrinkHash::build_abstraction() {
...
void MergeAndShrinkHash::initialize() {
    Timer timer;
...
int MergeAndShrinkHash::compute_heuristic(const State &state) {
...
/////////////////////////
/// DistirbutionHash Derivatives
/////////////////////////

unsigned int MergeAndShrinkHash::hash(const State& state) {
	return final_abstraction->get_abst_id(state);
}
unsigned int MergeAndShrinkHash::hash(const state_var_t* state) {
	printf("not implemented\n");
	return 0;

}
unsigned int MergeAndShrinkHash::hash_incremental(const State& state,
		const unsigned int parent_d_hash, const Operator* op) {
	return final_abstraction->get_abst_id(state);
}
std::string MergeAndShrinkHash::hash_name() {
	return "merge_and_shrink_hash";
}

static DistributionHash *_parse(OptionParser &parser) {
    ...
    parser.add_option<ShrinkStrategy *>(
        ...
        "shrink_clique(max_states=2000, max_states_before_merge=2000)");
    ...
    if (parser.dry_run()) {
        ...
        MergeAndShrinkHash *result = new MergeAndShrinkHash(opts);
        return result;
    }
}

static Plugin<DistributionHash> _plugin("merge_and_shrink", _parse);
