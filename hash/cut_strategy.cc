...
void RandomUpdating::cut(unsigned int var,
	...
	std::vector<std::vector<double> > transitions; // Edge matrix
	transitions.resize(size);
	for (int i = 0; i < size; ++i) {
		transitions[i].resize(size, 0.0);
	}
	...
	for (int i = 0; i < size; ++i) {
		...
		for (int j = 0; j < successors.size(); ++j) {
			transitions[i][successors[j]] = 1.0; // TODO: no edge weight
		}
		transitions[i][i] = 1.0; // TODO: edge weight
	}
	...
	double incumbent = sparsity->calculate_sparsity(transitions,
			which_structure);
	...
	for (int i = 0; i < ntest; ++i) {
		...
		double test_value = sparsity->calculate_sparsity(transitions,
				test_structure);
