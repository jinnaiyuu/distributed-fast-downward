...
////////////////////////////
template<>
class TokenParser<DistributionHash*> {
public:
	static inline DistributionHash *parse(OptionParser &p);
};

template<>
class TokenParser<CutStrategy*> {
public:
	static inline CutStrategy *parse(OptionParser &p);
};

template<>
class TokenParser<Sparsity*> {
public:
	static inline Sparsity *parse(OptionParser &p);
};
...
public:
	...
	template<class T>
	void add_option(std::string k, std::string h = "", std::string def_val = "",
	...
	void add_list_option(std::string k, std::string h = "",
			std::string def_val = "", const OptionFlags &flags = OptionFlags());
	...
	void document_note(std::string name, std::string note, bool long_text =
			false) const;
...
template<class T>
void OptionParser::add_option(std::string k, std::string h,
		std::string default_value, const OptionFlags &flags) {
	if (help_mode()) {
		DocStore::instance()->add_arg(parse_tree.begin()->value, k, h,
				TypeNamer<T>::name(), default_value, flags.mandatory);
		return;
	}
...
void OptionParser::add_list_option(std::string k, std::string h,
		std::string def_val, const OptionFlags &flags) {
...
T TokenParser<T>::parse(OptionParser &p) {
	...
	if ((str_stream >> std::boolalpha >> x).fail()) {
		p.error(
				"could not parse argument " + pt->value + " of type "
						+ TypeNamer<T>::name());
	}
	return x;
}
...
ScalarEvaluator *TokenParser<ScalarEvaluator *>::parse(OptionParser &p) {
	...
	if (Predefinitions<Heuristic *>::instance()->contains(pt->value)) {
		...
		return (ScalarEvaluator *) Registry<Heuristic *>::instance()->get(
				pt->value)(p);
	}
...
std::vector<T> TokenParser<std::vector<T> >::parse(OptionParser &p) {
	...
	if (pt->value.compare("list") != 0) {
		...
		for (ParseTree::sibling_iterator pti = first_child_of_root(
				*p.get_parse_tree());
				pti != end_of_roots_children(*p.get_parse_tree()); ++pti) {
			OptionParser subparser(subtree(*p.get_parse_tree(), pti),
					p.dry_run());
...
CutStrategy *TokenParser<CutStrategy *>::parse(OptionParser &p) {
	return lookup_in_registry<CutStrategy>(p);
}

Sparsity *TokenParser<Sparsity *>::parse(OptionParser &p) {
	return lookup_in_registry<Sparsity>(p);
}
